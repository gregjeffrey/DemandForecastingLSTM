...
for epoch in range(EPOCHS):
    ...
    for i in range(NUM_WINDOWS):
        ...
        window = training_inputs[index:index+WINDOW_LENGTH].unsqueeze(1).cuda()
        target = training_targets[index+24:index+WINDOW_LENGTH+24].cuda()
        
        # Zero gradients
        optimizer.zero_grad()

        # Update weights
        outputs = model(window).cuda()
